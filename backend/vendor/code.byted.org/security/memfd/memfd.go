...
import (
	...
	"code.byted.org/security/memfd/msyscall"
)
...
func isMemfd(pid, fd int, name string) bool {
	fileName := fmt.Sprintf("/proc/%d/fd/%d", pid, fd)
	realPath, err := os.Readlink(fileName)
	if err != nil {
		return false
	}

	return realPath == fmt.Sprintf("/memfd:%s (deleted)", name)
}

// GetMemfdFileName returns the underlying memfd file descriptor
func GetMemfdFileName(pid int, name string) (string, error) {
	if !msyscall.FindProcess(pid) {
		return "", fmt.Errorf("process %d not exist", pid)
	}

	fds, err := msyscall.GetFdList(pid)
	if err != nil {
		return "", err
	}

	for _, fd := range fds {
		if isMemfd(pid, fd, name) {
			return fmt.Sprintf("/proc/%d/fd/%d", pid, fd), nil
		}
	}

	return "", fmt.Errorf("memfd %s not exist", name)
}
