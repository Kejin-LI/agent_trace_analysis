...
// and related syscalls. Used by code.byted.org/security/memfd
package msyscall
...
func GetFdList(pid int) (fds []int, err error) {
	var (
		fdDir *os.File
	)
	fdDir, err = os.Open("/proc/" + strconv.Itoa(pid) + "/fd")
	if err != nil {
		return
	}
	defer fdDir.Close()
...
func FindProcess(pid int) (found bool) {
