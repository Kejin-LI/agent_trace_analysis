...
import (
	...
	"code.byted.org/security/sensitive_finder_engine/regexp/sparse"
)

type temState struct {
	dstateCache map[string]*dstate // dstate cache
	z1, z2      nstate             // two temporary nstates
}

func newTemState(prog *syntax.Prog) *temState {
	var s temState
	s.dstateCache = make(map[string]*dstate)

	s.z1.q.Init(uint32(len(prog.Inst)))
	s.z2.q.Init(uint32(len(prog.Inst)))

	return &s
}

func addq(q *sparse.Set, id uint32, flag syntax.EmptyOp, prog *syntax.Prog) {
	if q.Has(id) {
		return
	}
	q.Add(id)
	...
	case syntax.InstCapture, syntax.InstNop:
		addq(q, i.Out, flag, prog)
	case syntax.InstAlt, syntax.InstAltMatch:
		addq(q, i.Out, flag, prog)
		addq(q, i.Arg, flag, prog)
...
func (m *matcher) init(prog *syntax.Prog) error {
	...
	addq(&s.z1.q, uint32(prog.Start), syntax.EmptyBeginLine|syntax.EmptyBeginText, prog)
	s.z1.flag = flagBOL | flagBOT
	m.start = m.cache(&s.z1, s)

	s.z1.q.Reset()
	addq(&s.z1.q, uint32(prog.Start), syntax.EmptyBeginLine, prog)
	s.z1.flag = flagBOL
	m.startLine = m.cache(&s.z1, s)
...
func (m *matcher) computeNext(d *dstate, c int, temp *temState) *dstate {
	this, next := &temp.z1, &temp.z2
	this.dec(d.enc)
	...
	return m.cache(next, temp)
}

func (m *matcher) cache(z *nstate, temp *temState) *dstate {
	enc := z.enc()
	d := temp.dstateCache[enc]
	if d != nil {
		return d
	}

	d = &dstate{enc: enc}
	temp.dstateCache[enc] = d
	d.matchNL = m.computeNext(d, '\n', temp) == &dmatch
	d.matchEOT = m.computeNext(d, endText, temp) == &dmatch
	return d
}

func (m *matcher) match(b []byte, beginText, endText bool, temp *temState) (end int) {
	//	fmt.Printf("%v\n", m.prog)
...
func (m *matcher) matchString(b string, beginText, endText bool, temp *temState) (end int) {
	d := m.startLine
	if beginText {
		d = m.start
	}
...
func (m *matcher) matchStringIndex(b string, beginText, endText bool, temp *temState) (end int) {
	//var results []int
	d := m.startLine
	if beginText {
		d = m.start
