// Copyright 2011 The Go Authors.  All rights reserved.
// Use of this source code is governed by a BSD-style
// license that can be found in the LICENSE file.

// Package regexp implements regular expression search tuned for
// use in grep-like programs.
package regexp

import (
	"regexp/syntax"
	"sync"
)

func bug() {
	panic("codesearch/regexp: internal error")
}

// Regexp is the representation of a compiled regular expression.
// A Regexp is NOT SAFE for concurrent use by multiple goroutines.
type Regexp struct {
	Syntax *syntax.Regexp
	expr   string // original expression
	m      matcher
	prog   *syntax.Prog
	pool   sync.Pool
}

// String returns the source text used to compile the regular expression.
func (r *Regexp) String() string {
	return r.expr
}

// Compile parses a regular expression and returns, if successful,
// a Regexp object that can be used to match against lines of text.
func Compile(expr string) (*Regexp, error) {
	re, err := syntax.Parse(expr, syntax.Perl)
	if err != nil {
		return nil, err
	}
	sre := re.Simplify()
	prog, err := syntax.Compile(sre)
	if err != nil {
		return nil, err
	}
	if err := toByteProg(prog); err != nil {
		return nil, err
	}
	r := &Regexp{
		Syntax: re,
		expr:   expr,
		prog:   prog,
	}
	if err := r.m.init(prog); err != nil {
		return nil, err
	}
	r.pool = sync.Pool{
		New: func() interface{} {
			return newTemState(r.prog)
		},
	}
	return r, nil
}

func (r *Regexp) Match(b []byte, beginText, endText bool) (end int) {
	ts := r.pool.Get().(*temState)
	result := r.m.match(b, beginText, endText, ts)
	r.pool.Put(ts)
	return result
}

func (r *Regexp) MatchString(s string, beginText, endText bool) (end int) {
	ts := r.pool.Get().(*temState)
	result := r.m.matchString(s, beginText, endText, ts)
	r.pool.Put(ts)
	return result
}

func (r *Regexp) MatchStringIndex(s string, beginText, endText bool) (end int) {
	ts := r.pool.Get().(*temState)
	result := r.m.matchStringIndex(s, beginText, endText, ts)
	r.pool.Put(ts)
	return result
}
