...
func (d *Darts) Build(keywords []string) (*DoubleArrayTrie, *LinkedListTrie, error) {
	if len(keywords) == 0 {
	...
	d.Output = make(map[int]string, len(d.key))
...
func (d *Darts) fetch(parent *LinkedListTrieNode) (siblings [](*LinkedListTrieNode), err error) {
	...
	for i := parent.Left; i < parent.Right; i++ {
		...
		if cur != prev || len(siblings) == 0 {
			...
			tmpNode.Code = cur
			tmpNode.Left = i
			tmpNode.SubKey = make([]uint8, len(subKey))
...
func (d *Darts) insert(siblings [](*LinkedListTrieNode)) (int, error) {
	...
	for i := 0; i < len(siblings); i++ {
		...
		if len(newSiblings) == 0 {
			...
			d.Output[begin+int(siblings[i].Code)] = string(siblings[i].SubKey)
...
func (dat *DoubleArrayTrie) ExactMatchSearch(content []uint8, nodePos int) bool {
	b := dat.Base[nodePos]
	var p int
