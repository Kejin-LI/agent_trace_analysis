...
const (
	clientTimestampOffset  = 22
	gatewayTimestampOffset = 30
	flagOffset             = 38

	bodyHeaderSize       = 13 // Size of BodyHeader is 13
	comperssAlgOffset    = 8
	compressAlgLenOffset = 9
	metricCountOffset    = 33
	seriesCountOffset    = 37
)
...
// FlatMessage
// @Description: FlatMessage is an implementation of codec.Message.
// It is used by SDK or agent to encode metrics.It stream processes the metrics and their series.
type FlatMessage struct {
	...
	// meta info
	clientTimestamp            int64 // Client timestamp
	gatewayTimestamp           int64 // Gateway timestamp
	...
	currPosOfMetricHeader      int  // Record the start of a metric
	...
	// noCheck is a flag is for metrics sdk, since metrics sdk can make sure
	// 1. fields are all valid.
	// 2. tags all have valid characters and are sorted.
	// 3. contains dc and host tags.
	noCheck bool
}

// NewFlatMessage creates an empty flat message.
func NewFlatMessage(ops ...MessageOption) *FlatMessage {
	...
	if fm.isSeriesMergeable {
		fm.flatMetrics = metrics_codec.NewFlatMetrics(true)
	}
...
// Clear clears the metrics and series related to this message.
// Note that it resets the timestamps.
func (fm *FlatMessage) Clear() {
	fm.flatHeader = fm.flatHeader[:0]
	fm.flatBody = fm.flatBody[:0]
	fm.clientTimestamp = 0
	fm.gatewayTimestamp = 0
...
func (fm *FlatMessage) SetProperties(properties ...string) error {
	...
	for i := 0; i+1 < len(properties); i += 2 {
		...
		case metrics_codec.ProjectKey:
			// Project is the second tag.
			if len(fm.header.Tags) < 4 {
				fm.header.Tags = append(fm.header.Tags, k, v)
			} else {
				fm.header.Tags = append(fm.header.Tags, k, v)
				copy(fm.header.Tags[4:], fm.header.Tags[2:])
				fm.header.Tags[2], fm.header.Tags[3] = k, v
			}
		case metrics_codec.AccountKey:
			// Account is the first tag.
			if len(fm.header.Tags) < 2 {
				fm.header.Tags = append(fm.header.Tags, k, v)
			} else {
				fm.header.Tags = append([]string{k, v}, fm.header.Tags...)
			}
		default:
			fm.header.Tags = append(fm.header.Tags, k, v)
		}
...
func (fm *FlatMessage) SetClientTimestamp(ts time.Time) error {
	fm.clientTimestamp = metrics_codec.TimeUnixMilli(ts)
	return nil
}

func (fm *FlatMessage) GetClientTimestamp() time.Time {
	return metrics_codec.TimeFromUnixMilli(fm.clientTimestamp)
}

func (fm *FlatMessage) SetGatewayTimestamp(ts time.Time) error {
	fm.gatewayTimestamp = metrics_codec.TimeUnixMilli(ts)
	return nil
}

func (fm *FlatMessage) GetGatewayTimestamp() time.Time {
	return metrics_codec.TimeFromUnixMilli(fm.gatewayTimestamp)
}
...
func (fm *FlatMessage) AppendMetric(metricName string, mtype metrics_codec.MType, fields []metrics_codec.Field, ts time.Time) error {
	...
	if fm.currPosOfMetricHeader < len(fm.flatBody) {
		...
		metrics_codec.FillBackInt32(fm.flatBody, fm.currPosOfMetricHeader+metrics_codec.INT32_LEN,
			int32(fm.currMetricMeasurementCount))
	...
	fm.flatBody = metrics_codec.EncodeInt64(fm.flatBody, ts.Unix())
...
func (fm *FlatMessage) AppendMeasurement(hashcode int32, tags metrics_codec.FlatStructure, fieldIndices []int32, fieldValues ...float64) error {
	if len(fieldValues) > metrics_codec.MaxFieldCount {
	...
	if fm.isMultiFieldMetric &&
		(len(fieldIndices) == 0 && len(fieldValues) != fm.currentFieldCount) &&
		(len(fieldIndices) > 0 && len(fieldValues) != len(fieldIndices)) {
		return metrics_codec.ErrFieldIdxCountNotMatch
	}
	...
	if !fm.noCheck {
		...
		if fm.dc != "" && fm.host != "" {
			...
			*p = append(*p, tags...)
			var err error
			*p, hashcode, err = fm.addDCAndHostToTagBytes(*p, hashcode)
			if err != nil {
				return err
			}
			tagBytes = *p
	...
	if fm.isSeriesMergeable {
		return fm.flatMetrics.AppendMeasurement(hashcode, tagBytes, fieldIndices, fieldValues...)
	}

	fm.flatBody = appendMeasurement(fm.flatBody, hashcode, tagBytes, fieldIndices, fieldValues...)
...
func (fm *FlatMessage) AppendRawMeasurement(tags []metrics_codec.Tag, tagsAreSorted bool, fieldIndices []int32, fieldValues ...float64) error {
	if len(fieldValues) > metrics_codec.MaxFieldCount {
		return metrics_codec.ErrTooManyValues
	}
...
func (fm *FlatMessage) checkTags(tags []metrics_codec.Tag, sorted bool) ([]metrics_codec.Tag, int32) {
	...
	hashcode := metrics_codec.Fnv32aTags(tags...)
	return tags, int32(hashcode)
}

// tags bytes include the hashcode
func appendMeasurement(buf metrics_codec.FlatStructure, hashcode int32,
	tags metrics_codec.FlatStructure, fieldIndices []int32, values ...float64) metrics_codec.FlatStructure {
...
func (fm *FlatMessage) addDCAndHostToTagBytes(tagBytes []byte, hashcode int32) ([]byte, int32, error) {
	if len(tagBytes) < metrics_codec.INT32_LEN {
		return tagBytes, hashcode, metrics_codec.ErrTooShortForStringArray
	}

	tags, readLength, err := DecodeMetricMeasurementTs(tagBytes, 0)
	if err != nil {
		return nil, 0, err
	}

	if readLength != len(tagBytes) {
		return nil, 0, fmt.Errorf("tags length not match")
	}

	tags, hashcode = fm.checkTags(tags, true)
	tagBytes = tagBytes[:0]
	tagBytes = metrics_codec.Tags(tags).Encode(tagBytes)
...
func (fm *FlatMessage) finalize() error {
	...
	// fill back clientTimestamp.
	if len(fm.flatHeader) < clientTimestampOffset {
		return metrics_codec.ErrInvalidMessageHeader
	}
	metrics_codec.FillBackInt64(fm.flatHeader, clientTimestampOffset, fm.clientTimestamp)

	// gateway timestamp
	if len(fm.flatHeader) < gatewayTimestampOffset {
		return metrics_codec.ErrInvalidMessageHeader
	}
	metrics_codec.FillBackInt64(fm.flatHeader, gatewayTimestampOffset, fm.gatewayTimestamp)
	...
	fm.header.body.FillBack(fm.flatHeader, int32(len(fm.flatHeader)), int32(len(fm.flatBody)),
		int32(fm.totalMetricCount), int32(fm.totalMeasurementCount))
	return nil
}
