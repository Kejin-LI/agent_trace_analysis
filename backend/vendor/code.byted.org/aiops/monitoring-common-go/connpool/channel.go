...
func NewChannelPool(poolConfig *Config) (Pool, error) {
	...
	c := &channelPool{
		...
		lifeCounter:         poolConfig.LifeCounter,
...
BEGIN:
	...
	for {
		...
		default:
			...
			wrapConn = &idleConn{Conn: conn, born: time.Now(), t: time.Now(), counter: 1}
...
func (c *channelPool) Put(conn net.Conn) error {
	...
	// reach max used counter, close
	if c.lifeCounter > 0 && wrapConn.counter >= c.lifeCounter {
		c.mu.Unlock()
		return c.Close(conn)
	}
