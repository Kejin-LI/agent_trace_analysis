	...
	for _, path := range paths {
		for i := 0; i < 3; i++ {
			...
			if err == nil {
				conn.Close()
				if debugMetrics {
					logfunc("[DEBUG] gopkg/metrics: using socket path: %s\n", path)
				}
				return path
			}
...
func (w *metricsWriter) connect() (net.Conn, error) {
	if strings.HasPrefix(w.addr, "/") && strings.HasSuffix(w.addr, "seqpacket.sock") {
		return net.Dial("unixpacket", w.addr)
	}
...
func (s *sender) Send(ms []metricEntity) {
	...
	if s.w == ioutil.Discard {
		path := getSocketPath()
		if path != BlackholeAddr {
			s.batch = true
			s.w = getOrCreateWriter(getSocketPath())
		}
	}
	if !s.batch {
		p := wbufpool.Get().(*wbuf)
