...
4. Since HTTP headers are case-insensitive with a limited character set, to avoid key alteration when transmitting through a HTTP transport, we recommend **to name the keys in a CGI (Common Gateway Interface ) variable naming convention**. That is, using only upper case ASCII characters and separating words with underscores.
