...
var logPool = &sync.Pool{
	New: func() interface{} {
		return &Log{
			...
			psm:             make([]byte, 0, 16),
			kvlist:          make([]*codec.KeyValue, 0, 2),
			callDepthOffset: v2DefaultCallDepthOffset,
		}
	},
}
...
func init() {
	enableSecMark = DetermineRegion() == REGION_TTP || DetermineRegion() == REGION_GCP
...
type Log struct {
	...
	// TODO: it is bad sense to inject psm in a common structure
	psm []byte

	injectedLocation     string
	injectedFullLocation string
	callDepthOffset      int // callDepthOffset set by user.

	// callDepth offset by the bound logger of the log.
	// loggerDefaultCallDepthOffset != callDepthOffset means that the callDepth offset is modified by the user.
	loggerDefaultCallDepthOffset int
...
func newLog(level Level, logger *logger) *Log {
	...
	l.callDepthOffset = v2DefaultCallDepthOffset
...
func (l *Log) KV(key interface{}, value interface{}, ops ...kvOption) *Log {
	...
	default:
		kv := codec.NewOmniKeyValue(key, value)
...
func (l *Log) StrKV(key, value string) *Log {
	...
	kv := codec.NewStrKeyValue(key, value, true)
	l.kvlist = append(l.kvlist, kv)
	return l
}
...
func (l *Log) Emit() {
	...
	if enableSecMark {
		...
		l.trimPadding(true)
		l.kvlist = l.kvlist[:0]
		versionKV, _ := codec.NewKeyValue("current_version", currVersion)
	...
	case CurrGoroutine:
		...
		stackKV, _ := codec.NewKeyValue("stack", buffer.String(), true)
...
// setDefaultCallDepthOffset set the default callDepth offset of the log.
func (l *Log) setDefaultCallDepthOffset(callDepth int) *Log {
	if l == nil {
		return l
	}

	l.loggerDefaultCallDepthOffset = callDepth
	return l
}

// injectFileLocation will be used by tango compiler to inject file locations for each log.
func (l *Log) injectFileLocation(location string, fullPath bool) *Log {
	if l == nil {
		return l
	}

	if fullPath {
		l.injectedFullLocation = location
	} else {
		l.injectedLocation = location
	}
	return l
}

// isInjectedFileLocation checks whether the log is injected file location by compiler and it matches its default all depth.
func (l *Log) isInjectedFileLocation() bool {
	return l.callDepthOffset == l.loggerDefaultCallDepthOffset && l.logger.callDepth == defaultCallDepth &&
		(len(l.injectedLocation) > 0 || len(l.injectedFullLocation) > 0)
}
