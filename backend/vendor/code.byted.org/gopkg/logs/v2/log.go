...
func init() {
	enableSecMark = strings.Compare(env.IDC(), "useast5") == 0 && strings.Compare(env.Region(), "US-TTP") == 0
...
func (l *Log) Obj(o interface{}) *Log {
	if l == nil {
		return nil
	}
	switch v := o.(type) {
...
func (l *Log) Error(err error) *Log {
	if l == nil {
		return nil
	}
...
func (l *Log) KV(key interface{}, value interface{}) *Log {
	if l == nil {
		return nil
	}
...
func (l *Log) Emit() {
	...
	if enableSecMark {
		for _, kv := range l.kvlist {
			l.appendPaddingIfNecessary().Str("{{", kv.String(), "}}")
		}
		l.trimPadding(true)
		l.kvlist = l.kvlist[:0]
	...
	for i, _ := range l.logger.writers {
		if l.logger.writers[i].getLevel() > l.level {
			reader.Recycle()
			continue
		}
