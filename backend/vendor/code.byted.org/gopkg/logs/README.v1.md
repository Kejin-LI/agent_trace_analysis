...
After setting the mask mode, you could add a `mask` tag to the `field` of `struct`, then the field would be masked when log out.

Example like:

```
func main() {
    logs.SetMaskMode(true)
    ...
    logs.CtxInfo(context.Background(), "log session_key:%v", p)
    logs.Error("log session_key:430423199009102345", p)
    logs.Flush()
}
...
There are two Processors you can use. We recommend the first one.
```
logs.AddMaskEngineProcessor()

// or
logs.AddMaskFinderProcessor()
```

Learn more at API Doc.

### API Doc

[Golang 日志脱敏 SDK 使用文档](https://bytedance.feishu.cn/docs/doccnTPR8Rjs2vX4MDISYynWRWb#)

### Disable This Function

```
logs.SetMaskMode(false)
```

### Custom Mask Func

use `logs.SetMaskFunc("tag_name", mask_func)`

### Set Tag to IDL

```
struct PingResp {
    1: required string Msg (go.tag="mask:\"name\""),
...
[DataSecurity-303-数据分级安全保护措施基线要求](https://bytedance.feishu.cn/wiki/wikcnaFxMyg5o3EonWLqhQ9sech)


[脱敏SDK文档](https://bytedance.feishu.cn/docs/doccn5TiLGdslZVV1LV4fJ5teeN#Gtczh9)


[接口脱敏接入文档](https://bytedance.feishu.cn/docs/doccnrvdLWZx409LEA1ku8D76NX#)
